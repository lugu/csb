...
case class PilotManual(label: String, direction: Point, thrust: Double)
    extends Pilot
...
case class PilotCorrected(pilot: Pilot, pod: Pod, race: Race)(implicit
    val config: Config
) extends Pilot {
  ...
  def thrustCorrection = {
    ...
    val directionToDestinationAngle =
      (pod.destination - pod.position).radianWith(
        pilot.direction - pod.position
      )
    val directionToOrientationAngle =
      pod.angleToDest + directionToDestinationAngle
    ...
    if (pod.badCollision(race.enemy0))
      ...
      val ratio =
        1 - (abs(directionToOrientationAngle.degree) - maxAngle.degree) /
...
case class MetaPilot(pod: Pod, race: Race)(implicit val config: Config)
    extends Pilot {
  ...
  def chooseAvoid: Option[Pilot] = {
    if (
      !race.isFirstTurn && pod.detectCollision(friend) &&
  ...
  def chooseBoost: Option[Pilot] = {
    ...
    if (
      pod.boostCollide(race.friend(pod)) ||
      pod.boostCollide(race.enemy0) ||
      pod.boostCollide(race.enemy1)
    ) None
    else if (
      pod.boostAvailable &&
  ...
  def chooseSkip: Option[Pilot] = {
    ...
    if (
      pod.detectCollision(race.friend(pod))
      || pod.detectCollision(race.enemy0)
      || pod.detectCollision(race.enemy1)
    ) None
    else if (
      pod.stepsToDestination < 2 &&
      pod.checkSpeedCheckpoint(pod.destination) &&
      pod.speed.norm > skipSpeed
    ) Some(PilotSkip(pod, race))
    else None
  }
...
case class PilotHit(pod: Pod, race: Race, enemy: Pod)(implicit
    val config: Config
) extends Pilot {
  def label = "HIT"
  ...
  def direction = if (
    pod.position.distanceTo(enemy.destination) < enemy.distance
  )
    direction1
  else direction2
}

case class PilotWait(pod: Pod, race: Race, checkpoint: Point)(implicit
    val config: Config
) extends Pilot {
  ...
  def thrust = {
    if (dist > smallDistance) config.maxSpeedPilotWait
    else (dist / smallDistance) * config.maxSpeedPilotWait
  }
}

case class PilotAvoid(pod: Pod, other: Pod)(implicit val config: Config)
    extends Pilot {
  def label = "AVOID"

  def distanceBeforeMove = pod.position.distanceTo(other.position + other.speed)
  def distanceAfterMove =
    (pod.position + pod.speed).distanceTo(other.position + other.speed)

  // goal: direction opposite to the other
  def direction = pod.position + (other.position - pod.position).rotate(
    Degree(config.anglePilotAvoid)
  )

  def thrust =
    if (distanceBeforeMove > distanceAfterMove) 0 else config.maxSpeedPilotAvoid
}

case class PilotFight(pod: Pod, race: Race, enemy: Pod)(implicit
    val config: Config
) extends Pilot {
  val veryLarge = config.largeDistancePilotFight
  val pilot =
    if (
      pod.distanceToPod(enemy) < veryLarge ||
      enemy.score > race.laps * race.checkpoints.size - 2
    )
      PilotHit(pod, race, enemy)
    else if (
      pod.position.distanceTo(enemy.destination) <
...
case class PilotAttack(pod: Pod, enemy: Pod)(implicit val config: Config)
    extends Pilot {
...
case class PilotSkip(pod: Pod, race: Race)(implicit val config: Config)
    extends Pilot {
  def label = "SKIP"
...
case class Pilot2(pod: Pod, race: Race)(implicit val config: Config)
    extends Pilot {

  val skidAngle = Degree(config.skipAnglePilot2)
  val skidSpeed = config.skipSpeedPilot2

  val shallSkid: Boolean =
    if (
      pod.stepsToDestination < config.stepsToDestinationPilot2 &&
      pod.speedAngleToDest < skidAngle &&
      pod.speed.norm > skidSpeed
    ) true
    else false

  def skidDirection =
    pod.position + (pod.nextDestination(race) - pod.destination).normalize
...
case class Pilot1(pod: Pod, race: Race)(implicit val config: Config)
    extends Pilot {
  ...
  def newAngle1(angle: Angle) = Radian(
    sin(angle.radian) / config.angleDenumPilot1
  )
