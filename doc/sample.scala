...
case class Pilot1(pod: Pod, race: Race) extends Pilot {
    ...
    def newAngle1(angle: Angle) = Radian(sin(angle.radian)/2)
...
case class Pod(
    ...
    def updateDestination(race: Race): Pod = {
        val nextDest = nextDestination(race)
        val nextDistance = position.distanceTo(nextDest)
        val nextAngleToDest = angleToDest +
            destinationDirection.radianFrom(nextDest - position)
        Pod(position, nextDest, nextAngleToDest, speed)
    }
